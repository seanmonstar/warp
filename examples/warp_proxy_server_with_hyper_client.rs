...
pub async fn main() {
    ...
    let routes = warp::any()
        ...
        .and(warp::body::bytes())
        .map(|method: hyper::http::Method, path: warp::path::FullPath, query_params: String, headers: hyper::http::HeaderMap, body: hyper::body::Bytes| {
            let mut full_path = path.as_str().to_string();
            if query_params != "" { full_path = format!("{}?{}", full_path, query_params); }
...
async fn handler(mut request: hyper::Request<hyper::Body>) -> Result<hyper::Response<hyper::Body>, warp::Rejection> {
    // Make the client a global shareable client. This is just an example.
    let client = hyper::Client::new();

    // Manipulate the uri to your liking.
    *request.uri_mut() = "http://httpbin.org/ip".parse().expect("Failed to parse the uri");
